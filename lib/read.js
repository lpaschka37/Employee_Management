        ...
        .then((res) => {
            switch (res.department) {
                ...
                case ('Client Engagement'):
                    viewEmployeesInClientEngagement();
                    break;
                case ('Operations'):
...
const viewEmployeesInClientEngagement = () => {
    const query =
        'SELECT department.name, employee.first_name, employee.last_name FROM department JOIN role ON department.id = role.department_id JOIN employee ON employee.role_id = role.id WHERE department.name = "Client Engagement" ORDER BY department.name;';
    connection.query(query, (err, res) => {
        if (err) throw err;
        console.table(res);
        app.init();
        ...
        .then((res) => {
            switch (res.menu) {
                case ('Production'):
                    ...
                    break;
                case ('Client Engagement'):
                    viewUtilizedClientEngagementBudget();
                    break;
                case ('Operations'):
...
const viewUtilizedClientEngagementBudget = () => {
    const query =
        'SELECT department.name, employee.first_name, employee.last_name FROM department JOIN role ON department.id = role.department_id JOIN employee ON employee.role_id = role.id WHERE department.name = "Client Engagement" ORDER BY department.name;';
    connection.query(query, (err, res) => {
        if (err) throw err;
